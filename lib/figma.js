...
const createComponent = (component, imgMap, componentMap) => {
	...
	// END Create component file

	// Writing down component class into figmaComponent.js
	let doc = '';
	...
	const visitNode = (node, parent, lastVertical, indent) => {
		...
		// if (parent !== null) console.log('parent id: ', parent.id);
...
// DEFINES TEXT OBJECT
				...
				// console.log('--------------------');
				// console.log('Node Styles ID: ', node.styles);
	...
	const buildNode = (node) => {
		print(`<article className="${name}">`, '');
		
		const children = node.children;

		let buildChild = (child, index, indent) => {
			const className = `class-${(child.type == 'TEXT' && child.styles !== undefined) ? child.styles.text : child.id}`;
			let styles = null;

			// console.log(`:: child ${index}: `, child);
			
			styles = buildElementStyles(child);
			
			console.log(':: :: styles: ', styles);
			console.log(`---------------`)

			print(`<div className="${className}" style={${JSON.stringify(styles)}}>`, `${indent}`)

			if(child.type == 'TEXT') {

				console.log(':::: child: ', child)
				// console.log(':::: styleOverrideTable: ', child.styleOverrideTable)
				// console.log(':::: characterStyleOverrides: ', child.characterStyleOverrides)
				// console.log(':::: CHARACTERS: ', child.characters)
				print(`<p>${child.characters}</p>`, `${indent}${indent}`)
			} else {
				// print(`<div className="${className}" style={${JSON.stringify(styles)}}></div>`, `${indent}`)
			}
